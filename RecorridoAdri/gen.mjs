...
async function buildXlsx(d) {
  ...
  try {
    const wm = await grab(BASE + "/assets/walmart-spark-oficial.svg.png");
    const id = wb.addImage({ buffer: wm, extension: "png" });
    ws.addImage(id, { tl: { col: 13.2, row: 1 }, ext: { width: 38, height: 38 } });
  } catch (_e) {
  }
...
async function buildPdfDoc(d) {
  ...
  let spark = null;
  try {
    spark = await pdf.embedPng(await grab(BASE + "/assets/walmart-spark-oficial.svg.png"));
  } catch (_e) {
  }
